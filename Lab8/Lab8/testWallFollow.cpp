...
int wallFollow(int x, int y, FILE *fp, std::vector<Particle>& particles, double orientation, double magnitude, double variance,cv::Mat& image3,cv::Point& robot, int sonar_relative_orientation, /**/ std::vector<cv::Point> edge, std::vector<cv::Point> rocket, std::vector<cv::Point> dock, cv::Point arcLoad, int targetDist) {
	...
	int dist = getCM();
		...
		if(robot.x == userX && robot.y == userY) {
			...
			output = kp*err;
			driveleft = min(max(160+output,left-20.0), left+20.0);
			driveright = min(max(145+output,right-20.0), right+20.0);
...
int main( )
{
	...
	int dist;
	while (counts < 3) {
		dist = wallFollow(userX, userY, fp, particles, sonar_relative_orientation,  update_magnitude, update_variance, image3, robot, sonar_relative_orientation,/**/ edge, rocket, dock, arcLoad, 25);		
		if (dist > 50) counts++;
	}
